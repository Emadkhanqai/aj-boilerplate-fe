...
- `../DESIGN.md` — **read before writing any component**
...
- **DTOs come from `libs/data-access/api-types`** (generated from the upstream API's OpenAPI
  document). Never hand-duplicate an API model; never edit a generated file.
...
`npx nx run-many -t lint test build && npm run typecheck` passes, Playwright journeys for new routes are
green, axe-core reports no violations on changed screens, types match the upstream API
contract, and the change respects the git and SonarQube gates before any push.
