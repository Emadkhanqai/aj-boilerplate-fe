...
if [ -z "$KEY" ]; then
  ...
  if [ -f "$root/sonar-project.properties" ]; then
    KEY="$(sed -n 's/^[[:space:]]*sonar\.projectKey[[:space:]]*=[[:space:]]*//p' \
...
# TypeScript needs only the generic SonarScanner CLI — there is no compiler to
# wrap. Coverage has to exist before the scan, or Sonar records 0% on new code
# and the gate fails for the wrong reason.
if [ "${SONAR_RUN_SCAN:-0}" = "1" ]; then
  command -v npx >/dev/null 2>&1 || \
    block "SONAR_RUN_SCAN=1 but npx is not installed, so no scan could be produced."

  printf '[sonar-pre-push] running the scanner (SONAR_RUN_SCAN=1) — this takes a while...\n' >&2
  root="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
  ( cd "$root" \
    && npx --no-install nx run-many -t test --coverage \
