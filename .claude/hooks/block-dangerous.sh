...
# --- Destructive package / lockfile operations -------------------------------
if m '(^|[;&|(]|\s)npm\s+publish\b'; then
  block "npm publish pushes a package to a registry" \
        "Publishing is a release step a human performs deliberately, not an agent action."
fi
if m '(^|[;&|(]|\s)rm\s+[^;&|]*package-lock\.json'; then
  block "deleting package-lock.json discards the reproducible dependency graph" \
        "Change dependencies with 'npm install <pkg>' and commit the resulting lockfile diff."
fi

# --- Destructive browser-automation / test-baseline operations ---------------
if m '(--update-snapshots|-u(\s|$))' && m '(playwright|nx\s+e2e|web-e2e)'; then
  block "rewriting the Playwright visual baselines hides the failure it should surface" \
        "Look at the failing diff. A self-updating baseline asserts nothing."
fi

# --- Production deployment / hosting operations ------------------------------
if m '(^|[;&|(]|\s)(gcloud|az|aws|kubectl|helm|vercel|netlify|firebase)\b' \
   && m '(prod|production)'; then
  block "deployment command targets a production environment" \
        "Production changes go through the reviewed deployment pipeline, never an agent session."
fi
