...
#
#   .ts .tsx .mts .cts  ->  nx test <project that owns the file>
#
...
# Nothing to run for these. `api-types` is generated type declarations with no
# runtime behaviour, so a change there proves nothing until a consumer compiles.
case "$FILE" in
  */node_modules/*|*/dist/*|*/.angular/*|*/.nx/*|*/coverage/*|*/.git/*|*/api-types/*) exit 0 ;;
esac

# Only TypeScript sources map onto a test target.
case "$FILE" in
  *.ts|*.tsx|*.mts|*.cts) : ;;
  *) exit 0 ;;
esac
...
# hang a session waiting on a stuck test run.
run_bounded() {
...
# Projects without a `test` target (e.g. the e2e project, which /qa runs) are a
# silent skip rather than a spurious failure. Only checked when a JSON reader is
# available; without one, fall through and let Nx decide.
has_test_target() {
  _json="$(cd "$wsroot" && npx --no-install nx show project "$name" --json 2>/dev/null)"
  [ -z "$_json" ] && return 0
  if command -v jq >/dev/null 2>&1; then
    printf '%s' "$_json" | jq -e '.targets | has("test")' >/dev/null 2>&1
    return $?
  fi
  if command -v python3 >/dev/null 2>&1; then
    printf '%s' "$_json" | python3 -c \
      'import json,sys; sys.exit(0 if "test" in json.load(sys.stdin).get("targets",{}) else 1)' \
      2>/dev/null
    return $?
  fi
  return 0
...
if ! has_test_target; then
  printf '[run-affected-tests] Nx project "%s" has no test target — skipping\n' "$name" >&2
  exit 0
fi

out="$(cd "$wsroot" && run_bounded npx --no-install nx test "$name" 2>&1)"
rc=$?

if [ "$rc" -eq 0 ]; then exit 0; fi                      # quiet on success
if [ "$rc" -eq 127 ]; then
