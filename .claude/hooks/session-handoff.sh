...
if { has '\.claude/standards/' || has '\.editorconfig$' \
     || has 'eslint\.config' || has 'nx\.json$' || has 'tsconfig\.base\.json$'; } \
   && ! has 'CLAUDE\.md$'; then
  flag "**CLAUDE.md may be stale** — conventions, standards, or build configuration changed, but no \`CLAUDE.md\` was updated."
fi

# The API surface the app consumes changed, but the generated types did not.
if has 'libs/data-access/api-client/' && ! has 'api-types'; then
  flag "**Generated frontend types may be stale** — the API client changed but \`libs/data-access/api-types\` was not regenerated. Run \`/sync\`."
fi
if has 'api-types' && ! has '^docs/api/'; then
  flag "**API notes may be stale** — the generated types changed, but nothing under \`docs/api/\` was updated."
fi

# UI changed but the design contract did not.
if { has '\.html$' || has 'design/tokens\.css$' || has 'styles/app-preset\.ts$' \
     || has 'design/components\.css$'; } && ! has 'DESIGN\.md$'; then
  flag "**DESIGN.md may be stale** — templates or design tokens changed without a note in the visual contract."
fi

# Architectural change with no ADR.
if { has 'package\.json$' || has 'nx\.json$' || has 'eslint\.config'; } \
   && ! has '^docs/adr/'; then
  flag "**ADR may be missing** — a dependency or workspace-boundary change landed without a decision record in \`docs/adr/\`."
fi

# Tests.
if has '\.ts$' && ! has '\.spec\.ts$'; then
