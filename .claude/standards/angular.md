...
**Target:** Angular (latest LTS) + Nx + **PrimeNG**, at the repository root, workspace scope
`@aj-boilerplate`.

## Read this before building any UI

**`DESIGN.md`, at the repository root, is mandatory reading before writing a single
component.** It carries the design language — tokens, spacing scale, typography, colour roles,
component patterns, empty/loading/error treatments. An agent that builds UI without reading it
will produce something that looks nothing like the rest of the app, and that is treated as a
defect, not a nit. If `DESIGN.md` is still a template, fill it in *first*.
...
├── data-access/
│   ├── api-client/         # thin typed wrappers over the generated types + the HTTP interceptors
...
| `data-access/*` | `shared/util` | any `feature-*`, `shared/ui`, `auth` |
...
and lint failure is a build failure. This is also why `libs/data-access/api-client` never
imports `libs/auth` directly — it exposes injection tokens (`AUTH_TOKEN_PROVIDER`,
`TOKEN_REFRESHER`, `SESSION_EXPIRED_NOTIFIER`) that `auth` binds instead.
...
- **TanStack Query (`@tanstack/angular-query-experimental`)** is the async server-state layer:
  `injectQuery` / `injectMutation` against a single app-wide `QueryClient`. It owns retries,
  caching, invalidation, and the default error-toast wiring (see
  [`error-handling.md`](error-handling.md)) — do not hand-roll a parallel loading/error
  tracking mechanism next to it.
...
- Theme via PrimeNG design tokens defined in one place (`apps/web/src/styles/app-preset.ts`);
  never override component internals with `::ng-deep` scattered through feature styles.
...
- **Optimistic concurrency:** a form that edits an existing resource carries the `rowVersion` it
  loaded and sends it back on save; a `409` response means someone else changed the record —
  offer a reload, never silently retry the write. See
  `libs/feature-items/src/lib/item-form-page/item-form-page.ts` for the reference
  implementation and
  [`error-handling.md`](error-handling.md#409-conflict-optimistic-concurrency) for the
  client-side contract.

## API layer — generated, never hand-written

- **Types are generated from the backend OpenAPI document** into `libs/data-access/api-types`
  via `npm run generate:api` (`openapi-typescript`). **Hand-written HTTP clients are
  prohibited**, and so is hand-copying a backend DTO into a TypeScript interface.
...
- Feature code calls a service in `data-access/api-client`; it never injects `HttpClient`
  directly.
- **Consume versioned endpoints only** (`/api/v1/...`) — see
  [`api-versioning.md`](api-versioning.md).
- The `ApiResponse<T>` envelope is unwrapped **centrally** by `envelopeInterceptor`; feature
  code never reads `response.data` itself. See
...
- Handle **loading / error / empty / success** for every data view. An unhandled empty state is
  an incomplete feature. See [`error-handling.md`](error-handling.md).

## Security in the UI

- **Role-aware UI is UX, never security.** The API enforces every permission on every request;
  the UI only avoids showing the user a door they cannot open. Never implement a restriction by
  hiding alone — see [`owasp-security.md`](owasp-security.md) and `libs/auth/src/lib/roles.ts`,
  the single source of truth for role → capability mapping.
- **No `bypassSecurityTrustHtml` / `innerHTML` with user content** unless it is sanitised and
  the exception is reviewed. Angular escapes by default — keep it that way (see
  [`input-validation-sanitization.md`](input-validation-sanitization.md)).
- Tokens are held by the `auth` lib and attached by `authInterceptor`. No component reads a
  token, and nothing sensitive is persisted to `localStorage` without a reviewed reason.
...
- Type-checking (`npm run typecheck`) is part of the test surface.
- See [`testing.md`](testing.md) for the full detail, including MSW mocking.
...
```bash
npx nx run-many -t lint test build && npm run typecheck     # the local loop
npx nx affected -t test --base=origin/main       # what the hook runs
npx nx e2e web-e2e                                # Playwright
npm run generate:api                              # regenerate types from the backend OpenAPI doc
```

## Related

[`typescript.md`](typescript.md) · [`api-response-format.md`](api-response-format.md) ·
[`api-versioning.md`](api-versioning.md) · [`error-handling.md`](error-handling.md) ·
[`testing.md`](testing.md) · [`owasp-security.md`](owasp-security.md) ·
[`../templates/angular-component.md`](../templates/angular-component.md)
