...
profiles, the **new code** period on the main branch, the TypeScript / JavaScript / HTML / CSS
analysers, coverage import, security hotspots, and the Web API the pre-push hook queries.

### Where the pull-request safety net comes from instead

Since Sonar cannot see a pull request, the PR is gated by everything else in CI: lint,
typecheck, the production build, unit tests with a coverage threshold, Playwright E2E, Gitleaks
secret scanning, and the vulnerable-package audit (`npm audit`). SonarQube analyses `main`
after the merge. See [Continuous integration](#continuous-integration).
...
This is a JavaScript/TypeScript-only workspace (Angular + Nx) — there is exactly **one**
scanner to run, the generic SonarScanner CLI. No compiler-wrapped scanning session is needed
(that pattern only applies to compiled languages whose analyser needs to observe an actual
build, which does not apply here).
...
```bash
# Reporters are configured per project (project.json / vitest.config.mts), not on the
# command line: two projects run the vitest CLI, which has no --coverageReporters flag.
...
[`frontend-ci.yml`](../../.github/workflows/frontend-ci.yml) guards the Sonar job on
`github.event_name == 'push' && github.ref == 'refs/heads/main'` and says why in a comment.
Pull requests are gated by lint, typecheck, the production build, unit tests + coverage,
Playwright, and `npm audit` — see
...
[`git-approval-policy.md`](git-approval-policy.md) · [`../commands/quality-gate.md`](../commands/quality-gate.md) ·
