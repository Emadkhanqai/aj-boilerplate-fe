...
| `agents/` | Specialist subagent definitions — frontend, review, security, tests, gate. |
| `commands/` | Slash commands: the repeatable workflows (`/spec`, `/task`, `/implement`, `/qa`, …). |
| `workflows/` | Longer-form procedures a command references — new feature, API-contract change, review, gate, release. |
| `templates/` | Copy-paste starting points — ADR, pull request, Angular component. |
...
| `../.mcp.json` | MCP servers: Playwright, Chrome DevTools, SonarQube, Context7. |

### Commands vs. workflows

A **command** is what you type; a **workflow** is the procedure it follows. `/implement` and
`/sync` are the entry points; `workflows/new-feature.md` and `workflows/api-change.md` are the
full step-by-step behind them. Read the workflow when you want the reasoning, run the command
...
| `claude-code-workflows` | `wshobson/agents` | `frontend-mobile-development`, `unit-testing`, `security-scanning` |
...
`claude-code-workflows` ships 95 plugins; three stack-relevant ones are enabled to keep context
...
| `auto-format.sh` | PostToolUse `Edit\|Write` | no | Formats **only** the edited file: `.ts/.html/.css/.scss/.json/.md` → Prettier, plus `eslint --fix` for TypeScript. |
| `block-dangerous.sh` | PreToolUse `Bash` | **yes (exit 2)** | Refuses `rm -rf`, `git push --force`/`--force-with-lease`, `git reset --hard`, `git clean -fdx`, history rewriting, `npm publish`, deleting the lockfile, rewriting Playwright visual baselines, and commands that look like a production deploy or a credential sent to a remote host. Prints why. |
| `protect-files.sh` | PreToolUse `Edit\|Write` | **yes (exit 2)** | Refuses edits to `.env*` (templates excepted), the generated `libs/data-access/api-types/src/lib/types.ts`, the committed lockfile, `.claude/settings.json`, and key material. |
| `run-affected-tests.sh` | PostToolUse `Edit\|Write` | no (exit 1 on failure) | Runs `nx test <project>` for the touched project when a `.ts` file changes. Silent when nothing is affected. **Never exits 2**: it surfaces a failure without cancelling the edit. |
...
```
/qa          # everything local: lint, typecheck, test, build, audit, secret scan, Sonar
...
npm run typecheck                      # no project defines an Nx `typecheck` target;
                                       # the AOT production build is the other type gate
...
/spec  ──►  human approves  ──►  /task  ──►  /implement (one task at a time)
                                                  │
                                          /sync            (upstream API surface changed)
                                                  │
...
| The upstream API surface changed | [`workflows/api-change.md`](workflows/api-change.md) |
...
3. Link it from the agents that need it (`agents/frontend-agent.md`,
   `agents/frontend-engineer.md`) and from `commands/review.md` if reviewers should check it.
