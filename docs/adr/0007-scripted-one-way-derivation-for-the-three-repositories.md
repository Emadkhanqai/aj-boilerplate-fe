# ADR-0007: Derivation of the single-stack repositories is a committed one-way script
...
> Builds on [ADR-0004](0004-three-repository-split.md), which decided to publish three
> repositories and named "a derivation script" and "a drift check" as follow-on work.
> ADR-0004 is not superseded and its decision is unchanged. This ADR fills the gap it
> left open.
...
ADR-0004 split the boilerplate into three published repositories — `aj-boilerplate-fs`
...
against anything, because the rules exist only as prose in ADR-0004 and as habit.

Four forces shape what the fix can be:

- **The derived repositories are outputs, not workspaces.** ADR-0004 is explicit that work
...
- **There is no shared git history.** ADR-0004 dropped it deliberately, because the source
...
- **Not everything can be derived mechanically.** `README.md` describes two stacks.
  `CLAUDE.md` exists at the root and again inside each stack, and ADR-0004 says they
...
- **Promotes and drops by explicit rule.** `src/<stack>/*` moves to the root; the other
  ...
  governance files — is shared verbatim, which is what ADR-0004 requires.
...
- **Runs a cross-contamination check** after writing: no `src/frontend` in `be`, no
  ...
  every shared harness file present. This is the drift check ADR-0004 asked for, applied
...
- The rules of ADR-0004 stop being prose. `COMMON_DROP`, `BE_DROP`, `FE_DROP`, and
...
- A post-publication drift check that fetches the two published repositories and diffs the
  shared file set against `fs`. This is the half of ADR-0004's verification that the
...
The status quo. Rejected by evidence rather than by argument: ADR-0004 predicted drift,
...
than three copies. Rejected for the reason ADR-0004 gave — submodules confuse newcomers and
...
ADR-0004 rejected this as disproportionate and that remains true, but it is worth
...
Rejected in ADR-0004 and not reopened here. Recorded only because it is the alternative
that makes this ADR unnecessary, and it remains available: abandoning the three-repository
split would delete this entire class of problem. That trade is ADR-0004's to revisit.
...
- [ADR-0004](0004-three-repository-split.md) — the three-repository split this builds on
