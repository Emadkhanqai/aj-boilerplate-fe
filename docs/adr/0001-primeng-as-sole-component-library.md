# ADR-0001: PrimeNG is the only component library, and native form controls are banned
...
- `DESIGN.md` ships as a template the consuming project fills in before UI work, so tokens and
  layout rules are agreed before components are built.
...
dependency appearing in `package.json` is a violation of this ADR. `npx nx run-many -t lint --all`
and the axe-core suite in `npx nx e2e web-e2e` are the automated backstop.

## References

- [`.claude/standards/angular.md`](../../.claude/standards/angular.md) — the Angular and PrimeNG standard
- [`DESIGN.md`](../../DESIGN.md) — the visual contract this ADR defers the specifics to
- [`.claude/templates/angular-component.md`](../../.claude/templates/angular-component.md) — the component shape that follows from it
