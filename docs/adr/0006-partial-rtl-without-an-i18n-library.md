# ADR-0006: Direction switching without an i18n library — partial RTL, deliberately
...
`LanguageService` (`libs/shared/util/src/lib/language.service.ts`) holds an
...
- [ADR-0001: PrimeNG is the only component library](0001-primeng-as-sole-component-library.md) —
  PrimeNG components honour `dir` themselves, which is a meaningful part of why this was cheap
- [ADR-0005: The "What's new" modal is bespoke markup](0005-bespoke-whats-new-modal-as-a-primeng-exception.md) — the one
...
- `DESIGN.md` §5 — the logical-properties rule this makes load-bearing
- `libs/shared/util/src/lib/language.service.ts` — the signal this reacts to, and its
