...
([`.claude/hooks/session-handoff.sh`](../../.claude/hooks/session-handoff.sh)), which runs on
Claude Code's `Stop` event — that is, when a session ends.
...
- Any drift it noticed — [`CLAUDE.md`](../../CLAUDE.md), [`DESIGN.md`](../../DESIGN.md), or an
  [ADR](../adr/README.md) that no longer matches the code
...
If a handoff contradicts [`CLAUDE.md`](../../CLAUDE.md) or an [ADR](../adr/README.md), `CLAUDE.md`
and the ADR win. A handoff is a record of a moment, not a statement of the current convention. If
the contradiction means the documentation is out of date, fix the documentation — that is
condition 5 of [the Definition of Done](../definition-of-done.md).
