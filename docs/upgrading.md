...
> **Paths in this document are the full-stack repository's.** This is `aj-boilerplate-fe`, where
> the Nx workspace sits at the repository root — so read `src/frontend/apps/**` as `apps/**`,
> `src/frontend/libs/**` as `libs/**`, and so on. Rows describing `src/backend/**` have no
