...
> from *forgotten* — the same rule as the spec and ADR templates.
>
> **This report is blameless.** It names systems, changes, commits, and gaps. It does not name a
> person as a cause. "The guard resolved after the first render" is a finding; "X wrote the guard
> wrong" is not, and the difference is the difference between a report people write honestly and one
...
> the runtime configuration value, the regenerated contract, the dependency bump.
>
> **Distinguish the trigger from the cause.** A slow API response that exposes a component
> rendering before its data resolved is the trigger; the missing loading state is the cause.
> Stopping at the trigger produces a follow-up action about API latency and leaves the component
> in place.
>
...
>
> Walk the gates and say which one should have caught it and did not: the unit tests; the E2E
> journey; the accessibility check on critical routes; the module-boundary lint; the production
> build; SonarQube; Gitleaks; CodeQL; the dependency audit; the container image scan; code
> review; the staging smoke test. See
