...
- **Affected file or path**, as specific as you can be — ideally
  `libs/auth/src/lib/auth.service.ts:120`.
...
- **Anything that requires the consumer's own deployment configuration.** How your CDN is
  configured, which identity provider you wired up, what your ingress allows, what security
  headers the host in front of nginx adds.
- **The deliberately unconfigured placeholders** — the empty `OIDC_CLIENT_ID`,
  `OIDC_AUTHORITY`, and `OIDC_SCOPE` in `docker-compose.yml` and `docker/40-env.sh`, unset
  repository variables. They are empty because a value there would be either fake or dangerous,
  and no value can be right for every consumer.
- **Missing rate limits, WAF rules, or DDoS protection in front of a site nobody has deployed.**
  These are properties of a deployment, not of source code.
- **Anything that is a defect in the API this app calls rather than in this app.** This
  repository contains no server. A weak token issued by your identity provider, or an endpoint
  that authorises badly, belongs to whoever owns it — see "Related repositories" in `README.md`.
  What is in scope here is what this client does with what it receives.
- **Findings from a scanner run against the sample `libs/feature-items` slice, the MSW mock
  handlers, or the `dev` auth mode's role picker.** They are demonstration scaffolding and are
  meant to be deleted. That `AUTH_MODE=dev` contacts no identity provider is the documented
  purpose of that mode — though a path by which it survives into a production build, or is
  reachable without being asked for, is very much in scope. See below.
...
configuration — a cross-site scripting path through `innerHTML` or a bypassed sanitiser, a
token written to `localStorage` or to the console, a bearer token attached to a cross-origin
request by the interceptor, a route guard that renders before it decides, a `dev`-only code
path that survives the production build, or a lint rule that claims to enforce a library
...
| Gate | Where |
|---|---|
| **Gitleaks** secret scanning | `frontend-ci.yml`, and locally in `.claude/hooks/secret-scan.sh`. Config in [`.github/gitleaks.toml`](.github/gitleaks.toml) |
| **CodeQL** code scanning | `frontend-ci.yml` (TypeScript), `security-and-quality` query suite |
