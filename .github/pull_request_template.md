...
<!--
  ...
  If verification needs a real API rather than the MSW handlers, a particular role
  from the demo picker, or a seeded row, say so and say how to get one. A reviewer
  who has to reconstruct your setup will skip the step and approve on reading
  alone, which is how defects reach staging.
-->
...
**How do we roll it back?** <!-- Revert the merge commit? Is that enough? A cached bundle,
a runtime config change, or a dependency on a newer API contract may make the revert
insufficient on its own. -->

**Does anything have to happen in a particular order at deploy time?** <!-- the API released
first, a runtime config value set before the rollout, a feature flag flipped first -->
